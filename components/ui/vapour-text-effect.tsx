...
export default function VaporizeTextCycle({
  ...
  const fontConfig = useMemo(() => {
    ...
    return { fontSize, VAPORIZE_SPREAD, MULTIPLIED_VAPORIZE_SPREAD, font: `${font.fontWeight ?? 400} ${fontSize * globalDpr}px ${font.fontFamily}` };
  ...
  particles.forEach(particle => {
    ...
    if (shouldVaporize) {
      ...
      if (particle.shouldFadeQuickly) {
        ...
        if (currentVelocity > maxVelocity) { const s = maxVelocity / currentVelocity; particle.velocityX *= s; particle.velocityY *= s; }
